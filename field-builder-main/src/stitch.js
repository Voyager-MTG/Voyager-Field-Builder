...
function stitchLibraries(lib, newlib) {
	...
	for(let card in cards) {
		...
		console.log("before tokenscripts")
...
function pullColorIdentity(card) {
	...
	if(card.manaCost2)
		mash += card.manaCost2;
	let colors = mash.match(/[WUBRGI]/g)
	for(let c in colors) {
		if(!ci.includes(colors[c]))
			ci.push(colors[c])
	}
	// grab from color indicators
	let conv = {"White": "W", "Blue":"U", "Black":"B", "Red":"R", "Green":"G", "Silver":"I"}
	mash = card.color
	if(card.color2)
		mash += card.color2;
	colors = mash.match(/White|Blue|Black|Red|Green|Silver/g)
	...
	mash = mash.replace(/[*][(][^)][)][*]/g, "")
	colors = mash.match(/[{][WUBRGI]|[WUBRGI][}]/g)
	...
	conv = {"Plains": "W", "Island":"U", "Swamp":"B", "Mountain":"R", "Forest":"G", "Fault":"I"}
	mash = card.typeLine
	if(card.typeLine2)
		mash += card.typeLine2
	colors = mash.match(/Plains|Island|Swamp|Mountain|Forest|Fault/g)
	for(let cn in colors) {
		let c = conv[colors[cn]]
		if(!ci.includes(c))
			ci.push(c)
...
function colorFixer(card) {	
	console.log("very start")					//adds colors to 3+c cards since MSE has trouble with them
	if(card.color == "" && card.manaCost.match(/\}\{/i)) {
		...
		if(card.manaCost.match(/G/))
			colors.push("G");
		if(card.manaCost.match(/I/))
			colors.push("I");
		console.log("before arranging")
	...
	if(card.rulesText.match(/^This ?(creature|card|artifact|enchantment|permanent|token) is all colors./)) {
		card.color = "{White/Blue/Black/Red/Green/Silver} ";
	...
	if(card.rulesText.match(/Vp/) || card.manaCost.match(/Vp/)) {
		...
		let pris = 0;
		let cost_syms = card.manaCost.match(/(W|U|B|R|G|I|Vp)/g);
		...
		if(text_blocks) {
			for(let b in text_blocks) {
				if(!text_blocks[b].match(/Vp/))
					continue;
				let holder = [];
				let text_syms = text_blocks[b].match(/(W|U|B|R|G|I|Vp)/g);
