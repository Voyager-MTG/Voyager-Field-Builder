...
function tokenRegex(slim) {
	...
	let tokenColor = "(colorless|white|blue|black|red|green|silver)?(, (?:white|blue|black|red|green|silver),)?( and white| and blue| and black| and red| and green| and silver)? ?";
	...
	//let tokenMoreColors = "( (that's|that is|that are) (all colors|white, [^.]+|blue, [^.]+|black, [^.]+|red, [^.]+|green, [^.]+|silver, [^.]+))?"
}
...
function tokenPuller(c, shout) {
						...
						for(let i=0; i<bits.length; i++) {
							if(opts.includes(bits[i])) {
								if(bits[i] == "with" || bits[i] == "and has" || bits[i] == "that can't block") {
									...
									i++;
								}else if(bits[i+1].match(/ (all|white|blue|black|red|green|silver)/)){
	...
	console.log("done big match (idk what that is)");
	// EMBLEMS
	...
	console.log("done emblems")
	// AUDITION
	// let auditionMatch = thisCard.rulesText.match(/audition for (white|blue|black|red|green|silver)/i)
	// let auditionMatch2 = thisCard.rulesText.match(/audition for (white|blue|black|red|green) or for (white|blue|black|red|green|silver)/i)
	// if(auditionMatch) {
	...
	if(oracle.match(/(play|cast) ([^\n.]+ (from exile|exiled)|(one of )?those cards|them|it this turn|it until)/)) {
		tokens.push(["Can Be Cast From Exile", 1, "EXPT"]);
	}
	...
	if(cleanoracle.match(/(C|c)reate (a|two|three|four|five|X) trace/i)) {
		tokens.push(["Trace Reminder", 1, "Keyword: Trace"]);
		// tokens.push(["Embraced Representative", 1, "Keyword: Embrace"]);
	}
	if(cleanoracle.match(/{V/i)) {
		tokens.push(["Vertex Reminder", 1, "PVR"]);
	}
	if(cleanoracle.match(/(L|l)uminous/i)) {
		tokens.push(["Luminous", 1, "Keyword: Luminous"]);
	}
	if(cleanoracle.match(/(I|i)n your reserves/i)) {
		tokens.push(["Reserves", 1, "Keyword: Reserves"]);
	}
	// if(cleanoracle.match(/(L|l)iberate/i)) {
	// 	tokens.push(["Liberated Dragon", 1, "Keyword: Liberate"]);
	// }
	// if(cleanoracle.match(/(L|l)iberate/i)) {
	// 	tokens.push(["Liberated Dragon", 1, "Keyword: Liberate"]);
	// }
...
function tokenBuilding(flags) {
	...
	console.log("done loop 1adfadfs");
	...
	for(let c in library.cards) {
		...
		console.log("spellbook/conjure done");
	}
	console.log("done loop 2");
...
//		str = str.replace(/\/?Silver\/?/);
	...
	switch(str) {
		...
		case "{White/Silver} ":
		case "{Silver/White} ":
			tokenColor = "white and silver";
			tokenInit = "IW";
			break;
		case "{Blue/Silver} ":
		case "{Silver/Blue} ":
			tokenColor = "blue and silver";
			tokenInit = "IU";
			break;
		case "{Black/Silver} ":
		case "{Silver/Black} ":
			tokenColor = "Black and silver";
			tokenInit = "IB";
			break;
		case "{Red/Silver} ":
		case "{Silver/Red} ":
			tokenColor = "red and silver";
			tokenInit = "IR";
			break;
		case "{Green/Silver} ":
		case "{Silver/Green} ":
			tokenColor = "green and silver";
			tokenInit = "IG";
			break;
		...
		case "{Green/Blue/Red} ":
			tokenInit = "GUR";
			break;
		case "{Silver/White/Blue} ":
			tokenInit = "IWU";
			break;
		case "{Silver/Blue/Black} ":
			tokenInit = "IUB";
			break;
		case "{Silver/Black/Red} ":
			tokenInit = "IBR";
			break;
		case "{Silver/Red/Green} ":
			tokenInit = "IRG";
			break;
		case "{Silver/Green/White} ":
			tokenInit = "IGW";
			break;
		case "{Silver/White/Black} ":
			tokenInit = "IWB";
			break;
		case "{Silver/Black/Green} ":
			tokenInit = "IBG";
			break;
		case "{Silver/Green/Blue} ":
			tokenInit = "IGU";
			break;
		case "{Silver/Blue/Red} ":
			tokenInit = "IUR";
			break;
		case "{Silver/Red/White} ":
			tokenInit = "IRW";
			break;
		case "{White/Blue/Black/Red/Green/Silver} ":
...
function arrangeTokenColors(srcs) {
	console.log(srcs.toString());
	let colors = [];
	let map = {"white": "W", "blue": "U", "black": "B", "red": "R", "green": "G", "silver": "I", "W": "white", "U": "blue", "B": "black", "R": "red", "G": "green", "I": "silver"}
	for(let s in srcs) {
		// console.log(srcs[s].toString());
		if(!srcs[s])
			continue;
		if(srcs[s].match(/colorless/))
			return "colorless";
		let cm = srcs[s].match(/(white|blue|black|red|green|silver)/);
		if(cm) {
			let c = map[cm[1]];
...
function tokenNamer(card, skips) {
	...
	let c_map = {"W":"white", "U":"blue", "B":"black", "R":"red", "G":"green", "I": "silver"}
